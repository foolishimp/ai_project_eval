...
app.post('/api/run-test', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ 
            ...
            workingDir: (typeof workingDir !== 'undefined' ? workingDir : process.cwd()),
            fullPath: (typeof fullPath !== 'undefined' ? fullPath : testPath)
        });
    }
});
...
app.get('/api/version', async (req, res) => {
    ...
    try {
        // Try to read dashboard release version from deployment info
        const dashboardVersionPath = path.join(__dirname, 'dashboard-version.txt');
        try {
            dashboardVersion = require('fs').readFileSync(dashboardVersionPath, 'utf-8').trim();
        } catch {
            dashboardVersion = 'dev';
        }
        
        // Get project git commit info (where this dashboard is deployed)
        const { stdout: commitHash } = await execAsync('git rev-parse --short HEAD');
        const { stdout: commitDate } = await execAsync('git log -1 --format="%cd" --date=short');
        const { stdout: commitMessage } = await execAsync('git log -1 --format="%s"');
        
        projectVersion = `${commitHash.trim()} (${commitDate.trim()})`;
        
        res.json({
            dashboardVersion: dashboardVersion,
            projectCommit: commitHash.trim(),
            projectDate: commitDate.trim(),
            projectMessage: commitMessage.trim(),
            projectVersion: projectVersion,
            version: projectVersion,  // Keep for backward compatibility
            displayVersion: `Dashboard ${dashboardVersion} | Project ${projectVersion}`
        });
    } catch (error) {
        res.json({
            dashboardVersion: dashboardVersion,
            ...
            projectDate: 'unknown', 
            projectMessage: 'Git info not available',
            projectVersion: 'unknown',
            version: 'development',
            displayVersion: `Dashboard ${dashboardVersion} | Project development`
        });
    }
});
